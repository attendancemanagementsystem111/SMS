...
def get_total_classes():
    """Fetch the total number of classes from the database."""
    conn = sqlite3.connect("ams.db")  # Replace with your actual database name
    cursor = conn.cursor()
    
    cursor.execute("SELECT days FROM total ORDER BY id DESC LIMIT 1")  # Fetch the latest total classes
    result = cursor.fetchone()
    
    conn.close()
    
    return result[0] if result else 0  # Return 0 if no record exists
...
def add_days():
    try:
        ...
        new_days = int(data.get('days'))  # Convert input to integer
        ...
        if row:
            # Update existing row by adding new_days
            updated_days = row[0] + new_days
            cursor.execute("UPDATE total SET days = ? WHERE id = 1", (updated_days,))
        else:
            # Insert new row if table is empty
            updated_days = new_days
            cursor.execute("INSERT INTO total (id, days) VALUES (1, ?)", (updated_days,))

        conn.commit()
        conn.close()

        return jsonify({"message": f"Total days updated to {updated_days}!", "total_days": updated_days})
...
def get_total_days():
    try:
        ...
        total_days = row[0] if row else 0  # If no record, return 0
